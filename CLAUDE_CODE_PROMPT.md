...
> Paste this into your Claude Code terminal session at the start of every work session.
...
git config user.email "claude@phonefarm.local"
```
...
If you discover a bug inside a forbidden zone: **log it in CLAUDE_LOG.md** with the exact file and line. Do not fix it yourself. Inform the user.
...
Then commit the log: `git commit -m "docs: update CLAUDE_LOG"`
...
3. Log it in `CLAUDE_LOG.md` with `**Kilo must know:**`
...
- [ ] `CLAUDE_LOG.md` updated and committed
